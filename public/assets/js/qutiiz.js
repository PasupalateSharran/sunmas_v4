...
document.addEventListener('DOMContentLoaded', (event) => {
  ...
  const contents = document.querySelectorAll('.home, .aboutUs, .generalIndustries, .oilAndGas, .ourBusiness, .wasteWaterTreatment, .contactUs,.techSecAEditor,.techSecBEditor,.techSecCEditor,.techSecDEditor');
  ...
  tabs.forEach(tab => {
      tab.addEventListener('click', function(e) {
          ...
          console.log(`Clicked tab ID: ${this.id}, target content ID: ${targetId}`);
          const targetContent = document.getElementById(targetId);

          // Check if target content exists before displaying
          if (targetContent) {
              targetContent.style.display = 'block';
              console.log(`Showing content for: ${targetId}`);
          } else {
              console.error(`Content for tab ${this.id} not found.`);
          }
      });
  });
});
